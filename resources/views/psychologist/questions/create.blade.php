...
<div class="card">
    ...
    <div class="card-body pt-0">
        <form action="{{ route('psychologist.questions.store') }}" method="POST" enctype="multipart/form-data" id="kt_form_question" data-submit-method="fetch">
            @csrf
...
// Tidak ada lagi logika total skor; kita hanya memastikan satu jawaban benar di backend
            ...
            .then(async response => {
                ...
                try {
                    if (isJson) {
                        ...
                        // If status is 200, check if it's validation error or success
...
// Restore previews from localStorage on page load (only if validation failed)
document.addEventListener('DOMContentLoaded', function() {
    // Only restore previews if there are validation errors (old input exists)
    // Check if there's old input by checking if aspect_id has old value
    const hasOldInput = @json(old('aspect_id') !== null && old('aspect_id') !== '');

    if (!hasOldInput) {
        // No old input means user is starting fresh - clear all previews
        localStorage.removeItem('question_preview');
        for (let i = 0; i < 6; i++) {
            localStorage.removeItem(`choice_${i}_preview`);
        }
    ...
    // Only restore previews if validation failed (old input exists)
