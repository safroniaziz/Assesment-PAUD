...
<div class="app-container container-xxl">
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Tambah Team Member Baru</h3>
        </div>
        
        <form action="{{ route('psychologist.team-members.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                <div class="mb-10">
                ...
                <div class="mb-10">
                    ...
                    <input type="text" name="role" class="form-control @error('role') is-invalid @enderror" value="{{ old('role') }}" placeholder="e.g., Psikolog Anak, Lead Developer" required>
                    @error('role')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-10">
                    ...
                    <input type="number" name="order" class="form-control @error('order') is-invalid @enderror" value="{{ old('order') }}" min="1">
                    <div class="form-text">Urutan tampil (biarkan kosong untuk auto)</div>
                    @error('order')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
...
document.querySelector('form').addEventListener('submit', function(e) {
    ...
    Swal.fire({
        title: 'Konfirmasi',
        text: 'Apakah Anda yakin ingin menyimpan team member ini?',
        icon: 'question',
