    ...
    <div class="relative px-8 py-10">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div class="flex-1">
                <div class="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-4">
                    ...
                    <div class="flex bg-white/10 backdrop-blur-sm rounded-xl p-1">
                        <button id="gridView" class="view-toggle px-4 py-2 rounded-lg text-white font-medium transition-all duration-200 bg-white/20">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <!-- Export Button -->
                <button id="btnExport" class="inline-flex items-center space-x-3 bg-white/20 backdrop-blur-sm text-white px-6 py-3 rounded-2xl hover:bg-white/30 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 font-bold border border-white/20">
...
<div class="bg-white rounded-2xl shadow-lg p-6 mb-8 border border-gray-100">
    <div class="flex flex-col lg:flex-row gap-4">
        ...
        <div class="flex gap-3">
            <select id="yearFilter" class="px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all bg-white">
            ...
            <select id="sortBy" class="px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all bg-white">
                <option value="name">Urutkan: Nama</option>
                <option value="students">Urutkan: Jumlah Siswa</option>
                <option value="year">Urutkan: Tahun Ajaran</option>
            </select>
            
            <button id="resetFilters" class="px-4 py-3 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-all font-medium">
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl p-6 text-white shadow-xl">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold">{{ $classes->sum('students_count') }}</p>
            </div>
            <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
    ...
    <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white shadow-xl">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold">{{ $classes->count() > 0 ? round($classes->sum('students_count') / $classes->count(), 1) : 0 }}</p>
            </div>
            <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
    ...
    <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-6 text-white shadow-xl">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-orange-100 text-sm font-medium mb-1">Kelas Aktif</p>
                <p class="text-3xl font-bold">{{ $classes->where('students_count', '>', 0)->count() }}</p>
            </div>
            <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
...
<div id="classesContainer">
    ...
    <div id="gridViewContainer" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="class-card group relative bg-white rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3 border border-gray-100 overflow-hidden" data-class-id="{{ $class->id }}" data-name="{{ $class->name }}" data-year="{{ $class->academic_year }}" data-students="{{ $class->students_count }}">
