...
<div class="game-container h-screen flex flex-col p-2 md:p-4 relative overflow-hidden safe-area-inset">
    ...
    <div class="flex-shrink-0 text-center relative z-10 mt-2.5 md:mt-3 pb-safe">
        <button id="next-btn"
                data-is-last="{{ ($progress + 1) === $totalQuestions ? 'true' : 'false' }}"
                disabled
            ...
            <span class="relative z-10 flex items-center justify-center gap-2">
                <span id="next-btn-text">{{ ($progress + 1) === $totalQuestions ? 'Selesaikan' : 'Lanjutkan' }}</span>
                <i id="next-btn-icon" class="fas {{ ($progress + 1) === $totalQuestions ? 'fa-check' : 'fa-arrow-right' }} md:group-hover:translate-x-1 transition-transform duration-300 flex-shrink-0"></i>
            </span>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (isLastQuestion) {
                    ...
                    // Untuk soal selain terakhir, tampilkan info soal berikutnya dulu
                    Swal.fire({
                        title: 'Bagus!',
                        text: 'Yuk lanjut ke soal berikutnya.',
                        icon: 'success',
                        timer: 900,
                        showConfirmButton: false,
                        allowOutsideClick: false,
