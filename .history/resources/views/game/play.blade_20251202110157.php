...
<div class="game-container h-screen flex flex-col p-3 md:p-4 relative overflow-hidden">
    <!-- Elegant Header with Progress -->
        <div class="flex-shrink-0 mb-2 md:mb-3 relative z-10">
            <div class="bg-white/95 backdrop-blur-lg rounded-lg md:rounded-xl p-2 md:p-4 shadow-lg border border-gray-200">
                <!-- Mobile Layout -->
                <div class="md:hidden">
                    <div class="flex items-center justify-between mb-2">
                        <div class="flex items-center gap-2">
                            <div class="bg-gradient-to-br from-purple-500 via-indigo-500 to-pink-500 rounded-lg w-8 h-8 flex items-center justify-center text-white text-sm font-bold shadow-md">
                                {{ $progress + 1 }}
                            </div>
                            <div>
                                <p class="text-xs text-gray-800 font-bold">{{ $progress + 1 }} / {{ $totalQuestions }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="text-xs font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600">
                                {{ round((($progress + 1) / $totalQuestions) * 100) }}%
                            </p>
                        </div>
                    </div>
                    <div class="bg-gray-100 rounded-full h-2 overflow-hidden shadow-inner">
                ...
                <div class="hidden md:flex items-center justify-between gap-3 flex-wrap">
                    <div class="flex items-center gap-3 flex-shrink-0">
                        <div class="bg-gradient-to-br from-purple-500 via-indigo-500 to-pink-500 rounded-xl w-12 h-12 flex items-center justify-center text-white text-lg font-bold shadow-md flex-shrink-0">
                            {{ $progress + 1 }}
                        </div>
                    ...
                    <div class="text-right flex-shrink-0">
                        ...
                        <p class="text-base font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600">
                            {{ round((($progress + 1) / $totalQuestions) * 100) }}%
                        </p>
                    </div>
                </div>
    ...
    <div class="flex-1 flex flex-col md:grid md:grid-cols-2 gap-2 md:gap-4 overflow-y-auto relative z-10 min-h-0">
        ...
        <div class="flex items-center justify-center min-h-0 w-full flex-shrink-0 md:flex-shrink">
            <div class="bg-white rounded-lg md:rounded-xl p-3 md:p-5 shadow-xl w-full h-full flex flex-col border-2 border-gray-200 relative overflow-hidden min-h-0 max-w-full">
                ...
                <div class="text-center mb-2 md:mb-3 flex-shrink-0 relative z-10">
                    <div class="inline-flex items-center gap-1.5 md:gap-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white px-3 md:px-4 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-bold shadow-md whitespace-nowrap">
                        <i class="fas fa-star flex-shrink-0 text-xs"></i>
                        <span class="text-xs md:text-sm">Pilih jawaban yang benar!</span>
                    </div>
                </div>
                <div class="flex-1 flex items-center justify-center bg-gradient-to-br from-purple-50 via-indigo-50 to-pink-50 rounded-lg p-2 md:p-4 border border-purple-100 overflow-auto relative z-10 min-h-0 w-full">
        ...
        <div class="flex flex-col min-h-0 w-full flex-1 md:flex-shrink">
            <div class="bg-white/95 backdrop-blur-lg rounded-lg md:rounded-xl p-2 md:p-4 shadow-lg mb-2 md:mb-3 flex-shrink-0 border border-gray-200">
                <h3 class="text-center text-xs md:text-base font-bold text-gray-800 flex items-center justify-center gap-1.5 md:gap-2 whitespace-nowrap">
                    <i class="fas fa-hand-pointer text-purple-600 flex-shrink-0 text-xs md:text-base"></i>
                    <span class="text-xs md:text-base">Pilih Jawaban</span>
                </h3>
            </div>
            <div class="grid grid-cols-2 gap-2 md:gap-4 flex-1 min-h-0 overflow-y-auto w-full" id="choices-container">
                @foreach($currentQuestion->choices as $index => $choice)
                    <button type="button" 
                            class="choice-button group bg-white rounded-lg md:rounded-xl p-2 md:p-4 shadow-lg cursor-pointer border-2 border-gray-200 active:border-purple-400 md:hover:border-purple-400 transform active:scale-105 md:hover:scale-105 transition-all duration-300 relative overflow-hidden flex flex-col w-full min-w-0 touch-manipulation"
                            data-choice-id="{{ $choice->id }}">
                        <!-- Hover Overlay -->
                        <div class="absolute inset-0 bg-gradient-to-br from-purple-500/0 to-pink-500/0 md:group-hover:from-purple-500/5 md:group-hover:to-pink-500/5 transition-all duration-300 rounded-lg md:rounded-xl"></div>
                        
                        <!-- Selection Indicator -->
                        <div class="absolute top-1.5 right-1.5 md:top-2 md:right-2 w-5 h-5 md:w-7 md:h-7 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 md:group-hover:opacity-100 transition-all duration-300 z-10 shadow-lg flex-shrink-0">
                            <i class="fas fa-check text-xs"></i>
                        </div>
                        
                        <!-- Choice Image -->
                        <div class="flex-1 bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 rounded-lg p-1.5 md:p-3 mb-1.5 md:mb-2 border border-gray-100 flex items-center justify-center overflow-hidden md:group-hover:shadow-md transition-shadow duration-300 min-h-0 w-full">
                            <img src="{{ asset('storage/' . $choice->choice_image_path) }}" 
                                 ...
                                 class="w-full h-auto max-h-full object-contain rounded-md max-w-full">
                        </div>
                        
                        <!-- Choice Number Badge -->
                        <div class="text-center flex-shrink-0">
                            <div class="inline-flex items-center justify-center bg-gradient-to-br from-purple-500 via-indigo-500 to-pink-500 text-white rounded-full w-6 h-6 md:w-8 md:h-8 font-bold text-xs md:text-sm shadow-md md:group-hover:scale-110 transition-transform duration-300 flex-shrink-0">
                                {{ $index + 1 }}
                            </div>
                        </div>
                    </button>
    ...
    <div class="flex-shrink-0 text-center relative z-10 mt-2 md:mt-3">
        <button id="next-btn" 
                disabled
                class="bg-gradient-to-r from-green-400 via-green-500 to-emerald-600 text-white px-6 md:px-12 py-3 md:py-4 rounded-full text-sm md:text-lg font-bold shadow-xl disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 md:hover:scale-105 transition-all duration-300 transform relative overflow-hidden group w-full max-w-sm mx-auto whitespace-nowrap touch-manipulation">
