    ...
    <div class="bg-gradient-to-r from-gray-50 to-gray-100 px-8 py-6 border-b border-gray-200">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-4">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <!-- Search Bar -->
                <form method="GET" action="{{ $class ? route('teacher.classes.index') . '/' . $class->id . '/students' : route('teacher.students.index') }}" class="flex-1">
                    @if($class)
                    ...
                    <div class="relative">
                        ...
                        <input type="text" 
                               ...
                               class="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all bg-white text-sm font-medium">
                        @if(request('search'))
                            <a href="{{ $class ? route('teacher.classes.index') . '/' . $class->id . '/students' : route('teacher.students.index') }}" 
                ...
                <!-- Sort Dropdown -->
                <form method="GET" action="{{ $class ? route('teacher.classes.index') . '/' . $class->id . '/students' : route('teacher.students.index') }}" class="flex-shrink-0">
                    @if($class)
                        <input type="hidden" name="class" value="{{ $class->id }}">
                    @endif
                    @if(request('search'))
                    ...
                    <div class="relative">
                        <select name="sort_by" 
                                ...
                                class="appearance-none w-full sm:w-auto pl-4 pr-10 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all bg-white text-sm font-medium cursor-pointer">
                    ...
                    <input type="hidden" name="sort_order" value="{{ request('sort_order') == 'desc' ? 'desc' : 'asc' }}">
                </form>
                ...
                <form method="GET" action="{{ $class ? route('teacher.classes.index') . '/' . $class->id . '/students' : route('teacher.students.index') }}" class="flex-shrink-0">
                    ...
                    @endif
                    <input type="hidden" name="sort_order" value="{{ request('sort_order') == 'desc' ? 'asc' : 'desc' }}">
                    <button type="submit" 
                        ...
                        <i class="fas fa-{{ request('sort_order') == 'desc' ? 'sort-alpha-down' : 'sort-alpha-up' }} text-purple-600"></i>
                    </button>
                </form>
            </div>
        </div>
    ...
    <div class="bg-gradient-to-r from-gray-50 to-gray-100 px-8 py-6 border-t border-gray-200">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div class="text-sm text-gray-600 font-medium">
                Menampilkan <span class="font-bold text-purple-600">{{ $students->firstItem() ?? 0 }}</span> sampai <span class="font-bold text-purple-600">{{ $students->lastItem() ?? 0 }}</span> dari <span class="font-bold text-purple-600">{{ $students->total() }}</span> siswa
            </div>
            <div class="flex items-center space-x-2">
