                                ...
                                <tr id="session-details-{{ $session->id }}" class="hidden">
                                    <td colspan="4" class="px-6 py-0">
                                        <div class="py-4 bg-gradient-to-r from-gray-50 to-blue-50">
                                            ...
                                            <!-- Radar Chart - Professional Design like SIAMI -->
                                            <div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
                                                <div class="border-b border-gray-200 bg-gradient-to-r from-purple-50 to-pink-50 px-6 py-4">
                                                    <h5 class="text-lg font-bold text-gray-900 flex items-center">
                                                        ...
                                                        Visualisasi Performa per Aspek
                                                    </h5>
                                                </div>
                                                <div class="p-6">
                                            ...
                                            <div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
                                                ...
                                                <div class="p-6">
                                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                        @php
                                                            // Group results by aspect_id to avoid duplicates
kk                                                          $uniqueResults = $session->results->groupBy('aspect_id')->map(function ($group) {
                                                                return $group->first(); // Take first result if there are duplicates
                                                            })->values();
                                                            $colors = [
                                                                ['from' => 'from-purple-500', 'to' => 'to-purple-600', 'bg' => 'bg-purple-50', 'border' => 'border-purple-200'],
                                                                ['from' => 'from-pink-500', 'to' => 'to-pink-600', 'bg' => 'bg-pink-50', 'border' => 'border-pink-200'],
                                                                ['from' => 'from-blue-500', 'to' => 'to-blue-600', 'bg' => 'bg-blue-50', 'border' => 'border-blue-200'],
                                                                ['from' => 'from-indigo-500', 'to' => 'to-indigo-600', 'bg' => 'bg-indigo-50', 'border' => 'border-indigo-200'],
                                                                ['from' => 'from-violet-500', 'to' => 'to-violet-600', 'bg' => 'bg-violet-50', 'border' => 'border-violet-200'],
                                                                ['from' => 'from-fuchsia-500', 'to' => 'to-fuchsia-600', 'bg' => 'bg-fuchsia-50', 'border' => 'border-fuchsia-200'],
                                                                ['from' => 'from-rose-500', 'to' => 'to-rose-600', 'bg' => 'bg-rose-50', 'border' => 'border-rose-200'],
                                                                ['from' => 'from-cyan-500', 'to' => 'to-cyan-600', 'bg' => 'bg-cyan-50', 'border' => 'border-cyan-200'],
                                                            ];
                                                        @endphp
                                                        @foreach($uniqueResults as $index => $result)
                                                            @php
                                                            ...
                                                            <div class="group relative {{ $color['bg'] }} rounded-xl p-4 border-2 {{ $color['border'] }} hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                                                                ...
                                                                <div class="mb-2">
                                                                    <div class="text-xs font-bold text-gray-700 uppercase tracking-wide mb-1">{{ $result->aspect->name }}</div>
                                                                    <!-- Category Badge -->
                                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold {{ $catBadge['bg'] }} {{ $catBadge['text'] }}">
                                                                        {{ $catBadge['icon'] }} {{ $catBadge['label'] }}
                                                                    </span>
                                                                </div>
                                                                <div class="flex items-end justify-between">
                                                                    <div>
                                                                        <div class="text-4xl font-black bg-gradient-to-br {{ $color['from'] }} {{ $color['to'] }} bg-clip-text text-transparent">
                                                                            {{ $result->correct_answers }}
                                                                        </div>
                                                                        <div class="text-xs text-gray-500 font-semibold mt-1">dari {{ $result->total_questions }} soal</div>
                                                                    </div>
                                                                    <div class="text-right">
                                                                        <div class="text-sm font-bold bg-gradient-to-br {{ $color['from'] }} {{ $color['to'] }} bg-clip-text text-transparent">
                                                                            {{ round(($result->correct_answers / $result->total_questions) * 100) }}%
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
        ...
        options: {
            ...
            scales: {
                r: {
                    ...
                    ticks: {
                        stepSize: chartData.maxScore <= 10 ? 1 : Math.ceil(chartData.maxScore / 5),
...
@php
    // Group results by aspect_id to avoid duplicates
    $uniqueResults = $session->results->groupBy('aspect_id')->map(function ($group) {
        return $group->first(); // Take first result if there are duplicates
    })->values();
@endphp
window['chartData_{{ $session->id }}'] = {
    labels: [@foreach($uniqueResults as $result)'{{ $result->aspect->name }}'@if(!$loop->last), @endif @endforeach],
    scores: [@foreach($uniqueResults as $result){{ $result->correct_answers }}@if(!$loop->last), @endif @endforeach],
    maxValues: [@foreach($uniqueResults as $result){{ $result->total_questions }}@if(!$loop->last), @endif @endforeach],
    maxScore: {{ $uniqueResults->max('total_questions') ?? 10 }}
};
@endforeach
