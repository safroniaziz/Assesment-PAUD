                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse($students as $student)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            $latestSession = $student->latestSession;
                                            $hasResults = $latestSession && $latestSession->results && $latestSession->results->count() > 0;
                                        @endphp
                                        @if($hasResults)
                                            @php
                                                $avgPercentage = round($latestSession->results->avg('percentage'));
                                            @endphp
                                            <div class="text-sm font-bold text-purple-600">{{ $avgPercentage }}%</div>
                                            <div class="text-xs text-gray-500">
                                                Rata-rata dari {{ $latestSession->results->count() }} aspek
                                            </div>
                                        @else
                                            <span class="text-gray-400 text-sm">-</span>
                                        @endif
