    ...
    formElement.addEventListener('submit', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Add files from fileStorage if file inputs are empty
                if (fileStorage.questionImage && !formElement.querySelector('#question_image_input').files[0]) {
                    formData.set('question_image', fileStorage.questionImage);
                }

                // Add choice files from fileStorage
                Object.keys(fileStorage.choices).forEach(function(index) {
                    const fileInput = formElement.querySelector(`input[name="choices[${index}][image]"]`);
                    if (fileStorage.choices[index] && (!fileInput || !fileInput.files[0])) {
                        formData.set(`choices[${index}][image]`, fileStorage.choices[index]);
                    }
                });
                ...
                .then(async response => {
                    ...
                    if (response.status === 302 || response.status === 200) {
                        ...
                        if (location || (isHtml && !isJson)) {
                            // Get HTML response
                            const html = await response.text();
                            
                            // Check if response contains validation errors (look for error messages)
                            if (html.includes('Terjadi kesalahan validasi') || html.includes('validation') || html.includes('error')) {
                            ...
                            if (data.errors) {
                                ...
                                errorList = `<li class="mb-2"><i class="fas fa-exclamation-circle text-danger me-2"></i>${data.message}</li>`;
                            }

                            Swal.fire({
                                title: "Validasi Gagal",
                                html: `<div class="text-start"><ul class="list-unstyled mb-0">${errorList}</ul></div>`,
                                icon: "error",
                                buttonsStyling: false,
                                confirmButtonText: "Ok, got it!",
                                customClass: {
