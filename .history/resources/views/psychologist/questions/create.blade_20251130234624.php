    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            .then(response => {
                ...
                // If response is a redirect (status 0 with type 'opaqueredirect' or status 302)
                if (response.type === 'opaqueredirect' || response.status === 0) {
                    // This means redirect happened, get redirect URL from response
                    // For opaqueredirect, we need to check the final URL
                    clearFormData();
