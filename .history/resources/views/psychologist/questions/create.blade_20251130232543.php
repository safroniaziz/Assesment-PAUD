...
document.getElementById('kt_form_question').addEventListener('submit', function(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            // Restore question image if exists in storage but not in form
            const questionInput = form.querySelector('#question_image_input');
            if (fileStorage.questionImage && (!questionInput.files || questionInput.files.length === 0)) {
                formData.delete('question_image');
                formData.append('question_image', fileStorage.questionImage);
            }
            
            // Restore choice images if exists in storage but not in form
            Object.keys(fileStorage.choices).forEach(function(choiceIndex) {
                const choiceInput = form.querySelector(`input[name="choices[${choiceIndex}][image]"]`);
                if (choiceInput && (!choiceInput.files || choiceInput.files.length === 0) && fileStorage.choices[choiceIndex]) {
            ...
            .then(response => {
                if (response.redirected) {
                    ...
                    // Validation failed - get HTML and replace page
                    return response.text().then(html => {
                        // Create a temporary container to parse the response
                        const parser = new DOMParser();
                        const doc = parser.parseFromString(html, 'text/html');
                        
                        // Replace the form section with the new one
                        const newForm = doc.querySelector('#kt_form_question');
                        if (newForm) {
                            const currentForm = document.querySelector('#kt_form_question');
                            if (currentForm) {
                                ...
                                document.getElementById('kt_form_question').addEventListener('submit', arguments.callee);
                            }
                        } else {
                            // If form not found, reload the page
