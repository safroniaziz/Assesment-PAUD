    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            .then(response => {
                ...
                // If status is 0, it might be a CORS issue or redirect
                // Check response type
                if (response.status === 0 || response.type === 'opaqueredirect') {
                    // Redirect happened, go to index page
                    clearFormData();
                    window.location.href = '{{ route("psychologist.questions.index") }}';
                    return;
                }

                // If status is 200, it means validation failed - get HTML and replace page
                if (response.status === 200) {
                    return response.text().then(html => {
                        ...
                        // Replace page content with validation errors
                        document.open();
                        document.write(html);
                        document.close();
                    });
                ...
                // For other status codes, try to get response and handle
                return response.text().then(html => {
                    // If HTML contains redirect or success message, redirect
                    if (html.includes('psychologist/questions') || html.includes('Soal berhasil')) {
                        clearFormData();
                        window.location.href = '{{ route("psychologist.questions.index") }}';
                    } else {
