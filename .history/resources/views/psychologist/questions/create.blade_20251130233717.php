...
document.getElementById('kt_form_question').addEventListener('submit', function(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const form = document.getElementById('kt_form_question');
            if (form) {
...
function clearFormData(event) {
    ...
    // Clear session temp files via AJAX
    fetch('{{ route("psychologist.questions.index") }}', {
        method: 'GET',
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
        }
...
window.addEventListener('beforeunload', function() {
    ...
    if (!document.getElementById('kt_form_question').hasAttribute('data-submitting')) {
        clearFormData();
    }
});

// Clear form data when user navigates away (visibilitychange)
document.addEventListener('visibilitychange', function() {
    if (document.hidden) {
        // Page is hidden, but don't clear if form is being submitted
        if (!document.getElementById('kt_form_question').hasAttribute('data-submitting')) {
            // Don't clear on visibility change, only on actual navigation
        }
    }
});
