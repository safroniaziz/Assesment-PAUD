...
document.getElementById('kt_form_question').addEventListener('submit', function(e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (fileStorage.questionImage && !formData.has('question_image') || (form.querySelector('#question_image_input').files.length === 0 && fileStorage.questionImage)) {
            ...
            Object.keys(fileStorage.choices).forEach(function(choiceIndex) {
                ...
                if (choiceInput && choiceInput.files.length === 0 && fileStorage.choices[choiceIndex]) {
            ...
            .then(response => {
                if (response.redirected) {
                    ...
                    return response.text().then(html => {
                        // If validation failed, replace page content
                        document.open();
                        document.write(html);
                        document.close();
                    });
