...
class StudentController extends Controller
{
    public function index(Request $request, ClassRoom $class = null)
    {
        ...
        // Load count of assessment sessions for each student (must be before paginate)
        $query->withCount(['assessmentSessions' => function ($q) {
            $q->whereNotNull('completed_at'); // Hanya hitung yang completed
        }]);
        ...
        /** @var User $user */
        $user = Auth::user();
        $classes = $user ? $user->classes : collect();

        // If AJAX request, return JSON (use the same partial as initial render to keep table consistent)
        if ($request->ajax()) {
            return response()->json([
                'html' => view('teacher.students.partials.new_table', compact('students', 'class'))->render(),
