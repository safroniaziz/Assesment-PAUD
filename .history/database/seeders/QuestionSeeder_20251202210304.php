...
class QuestionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command->info('Menghapus data soal lama di database...');
        DB::table('question_choices')->delete();
        DB::table('questions')->delete();
        $this->command->info('✓ Data soal lama dihapus');
        ...
        $basePath = public_path('assets/assets/soal');
        ...
        foreach ($aspects as $aspect) {
            ...
            foreach ($questionDirs as $questionDir) {
                ...
                // Copy gambar soal ke storage publik (selalu simpan sebagai .jpg karena di-convert ke JPEG)
                $questionStoragePath = "assessments/questions/{$aspectSlug}-{$questionKey}.jpg";

                // Dapatkan path dengan fallback
                $questionRealPath = $questionImageFile->getRealPath() ?: $questionImageFile->getPathname();

                $this->copyToPublicStorage($questionRealPath, $questionStoragePath);
                ...
                foreach ($choiceFiles as $index => $file) {
                    // Selalu simpan sebagai .jpg karena di-convert ke JPEG
                    $choiceStoragePath = "assessments/choices/{$aspectSlug}-{$questionKey}-" . ($index + 1) . ".jpg";

                    // Dapatkan path dengan fallback
                    $choiceRealPath = $file->getRealPath() ?: $file->getPathname();

                    $this->copyToPublicStorage($choiceRealPath, $choiceStoragePath);

                    QuestionChoice::create([
                        'question_id' => $question->id,
                        'choice_image_path' => $choiceStoragePath,
                        // Secara default, tandai pilihan pertama sebagai jawaban benar.
                        // Guru/Psikolog bisa mengubahnya lewat dashboard jika perlu.
                        'is_correct' => $index === 0,
                        'order' => $index,
                    ]);
                }
    ...
    private function resizeImageTo4x3(string $sourcePath, string $targetPath): void
    {
        ...
        // Cek ekstensi file untuk menentukan tipe
        $ext = strtolower(pathinfo($sourcePath, PATHINFO_EXTENSION));

        // Load gambar sesuai ekstensi (tanpa getimagesize untuk hindari masalah extended attributes macOS)
        $sourceImage = null;
        switch ($ext) {
            case 'jpg':
            case 'jpeg':
            ...
            case 'webp':
                ...
                break;
            default:
                // Tipe tidak didukung, copy langsung dengan file_get_contents
        ...
        if (!$sourceImage) {
            // Gagal load gambar, copy langsung dengan file_get_contents (untuk handle extended attributes macOS)
            $content = @file_get_contents($sourcePath);
            if ($content !== false) {
                file_put_contents($targetPath, $content);
            }
