...
class QuestionSeeder extends Seeder
{
    ...
    /**
     * Try to copy real image from public/assets/soal if available,
     * otherwise fallback ke placeholder generator seperti sebelumnya.
     *
     * Konvensi:
     * - Seeder tetap menggunakan path seperti 'assessments/questions/kognitif-bentuk.jpg'
     * - Di folder `public/assets/soal` boleh disusun bebas (per aspek/per soal),
     *   yang penting NAMA FILE-nya sama (mis: `kognitif-bentuk.jpg`, `bentuk-segitiga.jpg`, dll).
     *
     * Saat seeder jalan:
     * - Jika menemukan file dengan nama yang sama di `public/assets/soal/**`,
     *   file itu akan di-copy ke storage publik dan dipakai di game.
     * - Jika tidak ketemu, seeder akan membuat gambar placeholder seperti sebelumnya.
     */
    private function generateImage(string $path, string $title, string $subtitle): void
    {
        try {
            ...
            $publicBase = public_path('assets/soal');

            $sourceFile = null;
            if (is_dir($publicBase)) {
                // Cari file dengan nama yang sama di seluruh subfolder assets/soal
                $files = File::allFiles($publicBase);
            ...
            // Pastikan direktori tujuan di storage publik ada
            ...
            if ($sourceFile && file_exists($sourceFile)) {
                // Jika file asli ditemukan di public/assets/soal, copy ke storage
                $targetPath = Storage::disk('public')->path($path);
                copy($sourceFile, $targetPath);
                return;
            }

            // --- Fallback: generate placeholder seperti sebelumnya ---
